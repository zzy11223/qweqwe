...
def pcap2npy4ISCX(dir_path_dict, save_path_dict):
    ...
    for category in dir_path_dict:
        ...
        for file in file_list:
            ...
            # 跳过空文件
            if PKT_COUNT == 0:
                print(f"{show_time()} 跳过空文件: {file_path}")
                continue
                
            # 跳过过长流（>10000个数据包）
            if PKT_COUNT > 10000:
                print(f"{show_time()} 跳过过长流: {file_path} ({PKT_COUNT} packets)")
                continue
                
            # 跳过空有效载荷流
            if all(pl == 0 for pl in payload_length):
                print(f"{show_time()} 跳过空有效载荷流: {file_path}")
                continue
