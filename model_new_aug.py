...
class MixTemporalGNN_SSL(nn.Module):
    ...
    def forward_pretrain(self, header_graph_data, payload_graph_data, header_mask, payload_mask):
        ...
        header_cl_loss = self.supcl_g(torch.cat(
                (F.normalize(temp1[mask12], p=2).unsqueeze(1), 
                 F.normalize(temp2[mask12], p=2).unsqueeze(1)), dim=1))
  
  
        payload_cl_loss = self.supcl_g(torch.cat(
                (F.normalize(temp3[mask34], p=2).unsqueeze(1), 
                 F.normalize(temp4[mask34], p=2).unsqueeze(1)), dim=1))
        ...
        seq_cl_loss = self.supcl(torch.cat(
            (F.normalize(representations, p=2).unsqueeze(1), 
             F.normalize(representations_aug, p=2).unsqueeze(1)), dim=1))
