...
class MixTemporalGNN_SSL(nn.Module):
    def __init__(self, num_classes, embedding_size=64, h_feats=128, dropout=0.2, downstream_dropout=0.0,
                 ...
                 hp_ratio=0.5, tau=0.07, gtau=0.07, proj_dim=256):
        ...
        self.seq_projection = ProjectionHead(
            in_dim=self.gcn_out_dim,
            hidden_dim=self.gcn_out_dim,
            out_dim=proj_dim
        )
        
        # 添加图级别的投影头
        self.header_projection = ProjectionHead(
            in_dim=self.gcn_out_dim,
            hidden_dim=self.gcn_out_dim,
            out_dim=proj_dim
        )
        
        self.payload_projection = ProjectionHead(
            in_dim=self.gcn_out_dim,
            hidden_dim=self.gcn_out_dim,
            out_dim=proj_dim
        )
    ...
    def forward_pretrain(self, header_graph_data, payload_graph_data, header_mask, payload_mask):
        ...
        # 通过投影头投影图表示
        proj_header_gcn_out1 = self.header_projection(header_gcn_out.reshape(-1, self.gcn_out_dim))
        proj_header_gcn_out2 = self.header_projection(aug_header_gcn_out.reshape(-1, self.gcn_out_dim))
        proj_payload_gcn_out1 = self.payload_projection(payload_gcn_out.reshape(-1, self.gcn_out_dim))
        proj_payload_gcn_out2 = self.payload_projection(aug_payload_gcn_out.reshape(-1, self.gcn_out_dim))
        
        # 计算图级对比损失（使用投影后的表示）
        header_cl_loss = self.supcl_g(g1=proj_header_gcn_out1, g2=proj_header_gcn_out2)
        payload_cl_loss = self.supcl_g(g1=proj_payload_gcn_out1, g2=proj_payload_gcn_out2)
        ...
        # 通过投影头投影序列表示
        proj_representations = self.seq_projection(representations)
        proj_representations_aug = self.seq_projection(representations_aug)
        
        # 计算序列级对比损失（使用投影后的表示）
        seq_cl_loss = self.supcl(g1=proj_representations, g2=proj_representations_aug)
